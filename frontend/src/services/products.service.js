...
const deleteProduct = (productId) => {
    ...
    return axios.delete(`http://localhost:8080/api/v1/manager/product/deleteProduct/${productId}`, { headers: authHeader() , method: 'DELETE',      
    'Content-Type' : 'application/json'})
    .then(response => {
  ...
  const updateProductPrice = (productId, updatedPrice) => {
    ...
    return axios.put("http://localhost:8080/api/v1/manager/product/updatePrice", newProductPrice, { headers: authHeader() , method: 'PUT',      
    'Content-Type' : 'application/json'})
    .then(response => {
