...
public class InitialConfiguration {
    ...
    private void addSampleOrders(OrderService orderService, ProductRepository productRepository) throws ParseException {
        ...
        ProductOrderDTO productOrderDTO3 = ProductOrderDTO.builder().product(productRepository.findByName("Tomato Bruschetta").get()).quantity(1).build();
        ProductOrderDTO productOrderDTO4 = ProductOrderDTO.builder().product(productRepository.findByName("Caramel Flan").get()).quantity(2).build();
        ProductOrderDTO productOrderDTO5 = ProductOrderDTO.builder().product(productRepository.findByName("Green Tea").get()).quantity(3).build();
        ProductOrderDTO productOrderDTO6 = ProductOrderDTO.builder().product(productRepository.findByName("Craft Beer").get()).quantity(2).build();
    ...
    private void addSampleProducts(ProductService productService) {
        ProductDTO starter1 = ProductDTO.builder().name("Tomato Bruschetta").description("Toasted bread with fresh tomato, garlic, and basil").category("Starter").unitPrice(5500.00).stock(20).unitCost(1000.00).newCategory(false).build();
        productService.addProduct(starter1);
        ProductDTO starter2 = ProductDTO.builder().name("Fried Calamari").description("Crispy calamari served with lemon sauce").category("Starter").unitPrice(6800.00).stock(15).unitCost(2500.00).newCategory(false).build();
        productService.addProduct(starter2);
        ProductDTO starter3 = ProductDTO.builder().name("Spanish Omelette").description("Potato and onion omelette with eggs").category("Starter").unitPrice(6200.00).stock(18).unitCost(2000.00).newCategory(false).build();
        productService.addProduct(starter3);
        ProductDTO starter4 = ProductDTO.builder().name("Shrimp Ceviche").description("Fresh shrimp ceviche with lime and cilantro").category("Starter").unitPrice(7500.00).stock(12).unitCost(3500.00).newCategory(false).build();
        productService.addProduct(starter4);
        ProductDTO starter5 = ProductDTO.builder().name("Italian Antipasto").description("Selection of cold cuts, cheeses, and olives").category("Starter").unitPrice(8900.00).stock(10).unitCost(5000.00).newCategory(false).build();
        productService.addProduct(starter5);
        ProductDTO productDTO1 = ProductDTO.builder().name("Veal Milanese with Fries").description("Veal milanese with french fries").category("Main Course").unitPrice(4500.00).stock(25).unitCost(4000.00).newCategory(false).build();
        productService.addProduct(productDTO1);
        ProductDTO productDTO2 = ProductDTO.builder().name("Margherita Pizza").description("Pizza with tomato, mozzarella, and basil").category("Main Course").unitPrice(8500.00).stock(30).unitCost(7000.00).newCategory(false).build();
        productService.addProduct(productDTO2);
        ProductDTO productDTO3 = ProductDTO.builder().name("Cheeseburger").description("Beef burger with cheddar cheese").category("Main Course").unitPrice(6500.00).stock(20).unitCost(1500.00).newCategory(false).build();
        productService.addProduct(productDTO3);
        ProductDTO productDTO4 = ProductDTO.builder().name("Caesar Salad").description("Lettuce, grilled chicken, croutons, and Caesar dressing").category("Main Course").unitPrice(7500.00).stock(15).unitCost(3500.00).newCategory(false).build();
        productService.addProduct(productDTO4);
        ProductDTO productDTO5 = ProductDTO.builder().name("Assorted Sushi").description("Assorted sushi with nigiri, sashimi, and rolls").category("Main Course").unitPrice(9500.00).stock(40).unitCost(7500.00).newCategory(false).build();
        productService.addProduct(productDTO5);
        ProductDTO productDTO6 = ProductDTO.builder().name("Spaghetti Carbonara").description("Spaghetti with egg, pancetta, and parmesan cheese").category("Main Course").unitPrice(7800.00).stock(18).unitCost(7000.00).newCategory(false).build();
        productService.addProduct(productDTO6);
        ProductDTO dessert1 = ProductDTO.builder().name("Tiramisu").description("Italian coffee and mascarpone cake").category("Dessert").unitPrice(7600.00).stock(15).unitCost(5500.00).newCategory(false).build();
        productService.addProduct(dessert1);
        ProductDTO dessert2 = ProductDTO.builder().name("Red Berry Cheesecake").description("Cheesecake with red berry sauce").category("Dessert").unitPrice(6900.00).stock(12).unitCost(5500.00).newCategory(false).build();
        productService.addProduct(dessert2);
        ProductDTO dessert3 = ProductDTO.builder().name("Chocolate Profiteroles").description("Pastries filled with chocolate cream and covered in chocolate").category("Dessert").unitPrice(8200.00).stock(18).unitCost(8000.00).newCategory(false).build();
        productService.addProduct(dessert3);
        ProductDTO dessert4 = ProductDTO.builder().name("Caramel Flan").description("Homemade flan with caramel sauce").category("Dessert").unitPrice(6300.00).stock(20).unitCost(6000.00).newCategory(false).build();
        productService.addProduct(dessert4);
        ProductDTO dessert5 = ProductDTO.builder().name("Mango Mousse").description("Fresh mango mousse with tropical fruits").category("Dessert").unitPrice(7100.00).stock(14).unitCost(6000.00).newCategory(false).build();
        productService.addProduct(dessert5);
        ProductDTO drink1 = ProductDTO.builder().name("Lemon Mojito").description("Rum, lime, sugar, and mint cocktail").category("Drink").unitPrice(7500.00).stock(25).unitCost(6000.00).newCategory(false).build();
        productService.addProduct(drink1);
        ProductDTO drink2 = ProductDTO.builder().name("Craft Beer").description("Local craft beer on tap").category("Drink").unitPrice(6800.00).stock(30).unitCost(5000.00).newCategory(false).build();
        productService.addProduct(drink2);
        ProductDTO drink3 = ProductDTO.builder().name("Raspberry Soda").description("Chilled raspberry soda").category("Drink").unitPrice(4900.00).stock(35).unitCost(2000.00).newCategory(false).build();
        productService.addProduct(drink3);
        ProductDTO drink4 = ProductDTO.builder().name("Espresso Coffee").description("Italian espresso coffee").category("Drink").unitPrice(3600.00).stock(40).unitCost(2500.00).newCategory(false).build();
        productService.addProduct(drink4);
        ProductDTO drink5 = ProductDTO.builder().name("Green Tea").description("Green tea with mint and honey").category("Drink").unitPrice(4200.00).stock(28).unitCost(3500.00).newCategory(false).build();
